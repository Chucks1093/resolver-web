...
import { usePaystack } from "@/hooks/home/usePaystack";
...
interface PricingCardProps {
	...
	buttonText?: string;
	buttonStyle?: string;
	handlePayment: (amount: number, plan: string) => void;
	index: number;
}
...
const MonthlyPlanIcon: React.FC = () => (
	<div className=' rounded-lg bg-blue-100 flex items-center justify-center p-1'>
		<img src='/icons/y-stack.svg' className='text-blue-600 h-12' />
	</div>
);

const YearlyPlanIcon: React.FC = () => (
	<div className=' rounded-lg bg-green-100 flex items-center justify-center p-1'>
		<img src='/icons/m-stack.svg' className='text-blue-600 h-12' />
	</div>
);
		...
		<TooltipProvider>
			...
			>
				...
				<motion.button
					...
					onClick={() =>
						plan !== "free" &&
						handlePayment(Number(price.replace(/,/g, "")), plan)
					}
				>
...
const Pricing = () => {
	...
	const handlePayment = (amount: number, plan: string) => {
		// console.log(profile);
		// if (!profile) {
		// 	return showToast.error("Please log in to use a plan");
		// }
		...
		return initializePayment({
			...
			metadata: { orderId: "test-order", plan },
		});
	};
		...
		<section className='max-w-7xl mx-auto flex flex-col items-center my-[8rem] px-4'>
			...
			>
				<p className='text-gray-500 text-sm'>
					All plans include unlimited video conversations with Nora.
					Cancel anytime.
				</p>
			</motion.div>
		</section>
