...
import UrlInputComponent from "@/components/dashboard/UrlInputComponent";
...
const CreateSession = () => {
	...
	// Handle URL changes from the UrlInputComponent
	const handleUrlsChange = (newUrls: any[]) => {
		setMaterials((prev) => ({
			...prev,
			urls: newUrls,
		}));
	...
	const handleSubmitSession = () => {
		if (!materials.textContent && materials.urls.length === 0)
			return showToast.error(
				"You need to add learning context or reference links to start this session"
			);
	...
	const buildContextWithUrls = (textContent: string, urls: any[]) => {
		let context = textContent;

		if (urls.length > 0) {
			context += "\n\nReference Links:\n";
			urls.forEach((url, index) => {
				context += `${index + 1}. ${url.title}: ${url.url}\n`;
			});
			context +=
				"\nPlease reference these links during our conversation when relevant.";
		}
	...
	const handleScheduleSession = async () => {
		...
		try {
			setIsLoading(true);
			showToast.loading("Generating lesson context");

			const contextWithUrls = buildContextWithUrls(
				materials.textContent,
				materials.urls
			);
			let generatedContext: string = contextWithUrls;
	...
	const handleCreateSession = async () => {
		if (!materials.textContent.trim() && materials.urls.length === 0) {
			showToast.error(
				"Please provide learning context or add reference links"
			);
			return;
		}
		...
		try {
			...
			let generatedContext = contextWithUrls;
			...
			<motion.div
				...
				<div className=''>
					<h1 className='text-3xl font-marlin font-bold text-gray-900 mb-2 flex items-center gap-3'>
						<Video className='size-12 text-blue-500' />
						Create Session
					</h1>
